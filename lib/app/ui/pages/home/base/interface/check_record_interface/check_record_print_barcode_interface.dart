...
///次品条码打印
mixin CheckRecordPrintBarcodeInterface on InvClassFrxNameInterface, GetxController {
  ...
  String frxNameMR = '';
  ///根据产品类别编码区分的打印模板名称列表
  Map<String, String> invClassFrxNameMapMR = {};
  ...
  Future<Map<bool, String>> _printCheckRecordBarcode({
    required String moRecordId,
    ...
    assert(taskModel != null || orderModel != null);
    ...
    if (barcodeMainList != null){
      ...
      assert(billCode != null);
    }

    ///是否是补打
    bool isRePrint = barcodeMainList != null;

    var checkRecordRes = await MoCheckRecordRepository().getModel(moRecordId);
    if (!checkRecordRes.isSuccess){
      return {false: '打印数据获取失败：${checkRecordRes.message}！'};
    }
    if (barcodeMainList == null){
      //region 准备打印数据 barcodeList
      BarcodeEntity barcode = BarcodeEntity();
      barcode.progid = checkRecordRes.data.progID;
      barcode.preProgid = checkRecordRes.data.progID;
      barcode.preId = checkRecordRes.data.moRecordId;
      barcode.preCode = checkRecordRes.data.billCode ?? '';
      barcode.preName = checkRecordRes.data.soCode ?? '';
      barcode.invID = checkRecordRes.data.invId ?? '';
      barcode.invCode = checkRecordRes.data.invCode ?? '';
      barcode.invName = checkRecordRes.data.invName ?? '';
      barcode.invStd = checkRecordRes.data.invStd ?? '';
      barcode.opId = checkRecordRes.data.opId ?? '';
      barcode.free1 = orderModel?.free1 ?? taskModel?.free1 ?? '';
      barcode.free2 = orderModel?.free2 ?? taskModel?.free2 ?? '';
      barcode.free3 = orderModel?.free3 ?? taskModel?.free3 ?? '';
      barcode.free4 = orderModel?.free4 ?? taskModel?.free4 ?? '';
      barcode.free5 = orderModel?.free5 ?? taskModel?.free5 ?? '';
      barcode.free6 = orderModel?.free6 ?? taskModel?.free6 ?? '';
      barcode.free7 = orderModel?.free7 ?? taskModel?.free7 ?? '';
      barcode.free8 = orderModel?.free8 ?? taskModel?.free8 ?? '';
      barcode.free9 = orderModel?.free9 ?? taskModel?.free9 ?? '';
      barcode.free10 = orderModel?.free10 ?? taskModel?.free10 ?? '';
      barcode.empty1 = taskModel != null ? taskModel.opDescription : checkRecordRes.data.lineCode; ///材料 OR 产线 Code
      barcode.empty2 = checkRecordRes.data.productDate.toString();
      barcode.empty3 = checkRecordRes.data.emploee ?? '';
      barcode.empty4 = checkRecordRes.data.mtoNo ?? '';
      barcode.empty5 = checkRecordRes.data.mtoSeq?.toString();
      barcode.empty6 = taskModel?.invWhName ?? '';
      barcode.empty7 = checkRecordRes.data.deviceCode ?? '';
      barcode.empty10 = orderModel?.whName ?? taskModel?.whName ?? '';
      barcode.teamCode = checkRecordRes.data.teamCode ?? '';
      barcode.batch = checkRecordRes.data.batch;
      barcode.productDate = checkRecordRes.data.productDate;
      barcode.billCode = billCode;
      barcode.quantity = checkRecordRes.data.disabledQty ?? 0;
      barcode.boxQuantity = checkRecordRes.data.disabledQty ?? 0; //单箱件数
      barcode.piece = checkRecordRes.data.disabledQty ?? 0;
      ...
      var barcodeListRes = await BarcodeMainRepository().getPageList(
        PageConfig(
          ...
          queryData: {'preId': checkRecordRes.data.moRecordId}
        )
      );
    ...
    String frxName = '';
    Map<String, String> invClassFrxNameMap = {};
    if (checkRecordRes.data.progID == 811010){
      frxName = this.frxName;
      invClassFrxNameMap = this.invClassFrxNameMap;
    }
    else if (checkRecordRes.data.progID == 811013){
    ...
    switch (printType){
      case 'serverPrint':
        ...
        barcodeMainList.forEach((element) {
          ...
          map['DeviceCode'] = checkRecordRes.data.deviceCode;
          map['DeviceName'] = checkRecordRes.data.deviceName;
          map['MouldCode'] = checkRecordRes.data.mouldCode;
          map['MouldName'] = checkRecordRes.data.mouldName;
          map['Output'] = taskModel?.output; ///标准模穴
          map['AvailOutput'] = taskModel?.availOutput; ///实际模穴
          map['OutCycle'] = taskModel?.outCycle; ///标准周期
          map['ActualCycle'] = taskModel?.actualCycle; ///实际周期
          map['Principal'] = taskModel?.principal; ///模具负责人
          map['TeamCode'] = checkRecordRes.data.teamCode;
          map['TeamName'] = checkRecordRes.data.teamName;
          map['DepCode'] = checkRecordRes.data.depCode;
          map['DepName'] = checkRecordRes.data.depName;
          map['Emploee'] = checkRecordRes.data.emploee;
          map['Qty'] = checkRecordRes.data.disabledQty; ///报次品数量
          map['ComDefects'] = checkRecordRes.data.comDefects;
          ...
          map['Free1'] = orderModel?.free1 ?? taskModel?.free1 ?? '';
          map['Free2'] = orderModel?.free2 ?? taskModel?.free2 ?? '';
          map['Free3'] = orderModel?.free3 ?? taskModel?.free3 ?? '';
          map['Free4'] = orderModel?.free4 ?? taskModel?.free4 ?? '';
          map['Free5'] = orderModel?.free5 ?? taskModel?.free5 ?? '';
          map['Free6'] = orderModel?.free6 ?? taskModel?.free6 ?? '';
          map['Free7'] = orderModel?.free7 ?? taskModel?.free7 ?? '';
          map['Free8'] = orderModel?.free8 ?? taskModel?.free8 ?? '';
          map['Free9'] = orderModel?.free9 ?? taskModel?.free9 ?? '';
          map['Free10'] = orderModel?.free10 ?? taskModel?.free10 ?? '';
          map['InvMnemCode'] = invMnemCode;
          map['PackingWeight'] = checkRecordRes.data.packingWeight;
          map['LineCode'] = checkRecordRes.data.lineCode;
          map['LineName'] = checkRecordRes.data.lineName;
        ...
        AppRepository().downloadFile(
          FormRepository().getPrintUrl(
            invClassFrxNameMap[orderModel?.invCCode] ?? frxName,
            null, 'pdf',
          ),
          parames: jsonStr,
  ...
  Future<Map<bool, String>> Function({
    ...
    String deviceAddCode,
    String invMnemCode,
  }) get printCheckRecordBarcode => _printCheckRecordBarcode;
