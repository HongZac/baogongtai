...
import 'package:desktop/app/ui/widget/fluent_ui/tree_view/null_of_check_box.dart';
...
class TreeViewItem with Diagnosticable {
  ...
  TreeViewItem({
    ...
    this.loadingWidget,
    this.lazy = false,
  })  : children = children ?? <TreeViewItem>[],
        expanded = expanded ?? (children?.isNotEmpty ?? false),
        _anyExpandableSiblings = false,
        focusNode = focusNode ?? FocusNode();

  factory TreeViewItem.TreeModelToTreeViewItem(BuildContext context, {
    required TreeModel item,
    List<String> selectedItemIdList = const [],
  }) {
    return TreeViewItem(
      key: ValueKey(item.id),
      content: Text(item.text ?? '', style: Theme.of(context).textTheme.bodyLarge,),
      value: item,
      expanded: false,
      selected: selectedItemIdList.contains(item.id),
      children: List.generate(item.childNodes.length, (index) {
        return TreeViewItem.TreeModelToTreeViewItem(
            context, item: item.childNodes[index],
  ...
  void updateSelected(bool deselectParentWhenChildrenDeselected, {TreeViewSelectionMode? selectionMode}) {
    if (selectionMode == TreeViewSelectionMode.singleIncludeParent){
      bool hasNull = false;
      bool hasFalse = false;
      bool hasTrue = false;
      ...
      if (!deselectParentWhenChildrenDeselected && (children.isEmpty || (!hasNull && hasFalse && !hasTrue))) {
        if (selected == null && children.isEmpty) { /// should not be possible unless children were removed after the selected was updated previously...
          selected = true;
        }
        else if (selected == true) { /// we're now only in a partially selected state
          selected = null;
        }
      }
      else {
        if (selected == null || !selected!){ ///如果该项本身是 false（未选中状态），则执行
          if (hasTrue || hasNull){
            selected = null;
          }
          else {
  ...
  List<TreeViewItem> build({TreeViewItem? parent}) {
    if (isNotEmpty) {
      ...
      for (final item in [...this]) {
        ...
        final itemAnyExpandableSiblings = item.children.any((i) => i.isExpandable);
...
typedef TreeViewSelectionChangedCallback = Future<void> Function(Iterable<TreeViewItem> selectedItems)?;

/// A callback that receives a notification that an item has been invoked.
///
/// Used by [TreeView.onItemInvoked]
typedef TreeViewItemInvoked = Future<void> Function(TreeViewItem item, TreeViewItemInvokeReason reason);

/// A callback that receives a notification that an item
/// received a secondary tap.
///
/// Used by [TreeView.onSecondaryTap]
typedef TreeViewItemOnSecondaryTap = void Function(TreeViewItem item, TapDownDetails details, );
...
typedef TreeViewItemOnExpandToggle = Future<void> Function(TreeViewItem item, bool getsExpanded, );
...
class _TreeViewState extends State<TreeView> with AutomaticKeepAliveClientMixin {
  ...
  void _buildItems() {
    ...
    widget.items.executeForAll(
          (item) => item.executeForAllParents((parent) => parent?.updateSelected(widget.deselectParentWhenChildrenDeselected, selectionMode: widget.selectionMode)),
    );
  ...
  Widget loadingWidget(){
    return widget.loadingWidget ?? SpinKitCircle(
      color: widget.contentStyle?.color ?? (Theme.of(context).brightness == Brightness.light ? Colors.black87 : Colors.white),
  ...
  Widget build(BuildContext context) {
    ...
    TextStyle contentStyle = widget.contentStyle ?? Theme.of(context).textTheme.bodyLarge ?? const TextStyle(fontSize: 14);
    return FocusTraversalGroup(
      policy: WidgetOrderTraversalPolicy(),
      child: ConstrainedBox(
        constraints: const BoxConstraints(minHeight: 28.0),
        child: ListView.builder( ///如果shrinkWrap为true，那么我们默认不使用主滚动控制器（在这种情况下通常不需要任何控制器）
          ...
          _TreeViewItem(
            ...
            onExpandToggle: () {},
            loadingWidgetFallback: loadingWidget(),
            ...
            return _TreeViewItem(
              ...
              onSelect: () async {
                ...
                switch (widget.selectionMode) {
                  ...
                  case TreeViewSelectionMode.singleIncludeParent:
                    setState(() {
                      //_items.executeForAll((element) {
                      ...
                      if (item.parent != null){
                        item.parent!.selected = item.selected! ? null : false;
                      }
                    });
                  ...
                  case TreeViewSelectionMode.multiple:
                    setState(() {
                      final newSelectionState = item.selected == null || item.selected == false;
                      item.setSelectionStateForMultiSelectionMode(
                        newSelectionState,
                        widget.deselectParentWhenChildrenDeselected);
                    });
                    if (onSelectionChanged != null) {
                      final selectedItems = widget.items.selectedItems(widget.includePartiallySelectedItems);
  ...
  Future<void> invokeItem(TreeViewItem item, TreeViewItemInvokeReason reason) async {
    await Future.wait([
                  ...
                  children: [
                    Container(
                      decoration: BoxDecoration(
                        //color: selected && selectionMode == TreeViewSelectionMode.single
                      ...
                      child: Row(
                        ...
                        children: [
                          if (item.isExpandable)
                            if (item.loading) ///加载动画
                              ...
                              MineIconButton(
                                ...
                                iconSize: iconSize,
                                tooltip: item.expanded ? '收起' : '展开',
                              ),
                          //else ///添加填充以使相同深度级别的所有项目具有相同的缩进，无论它们是否可扩展
                          //  const Padding(padding: EdgeInsetsDirectional.only(start: 24.0)),
                          ...
                          //if (selectionMode == TreeViewSelectionMode.multiple) ///复选框
                            Padding(
                              padding: EdgeInsetsDirectional.only(
                                end: narrowSpacing ? 0.0 : _whiteSpace,
                              ),
                              child: NullOfCheckbox(
                                size: checkboxSize,
                                iconSize: iconSize,
