...
class MyTonalPalette {
  ...
  MyTonalPalette._fromHct(Hct hct)
      : _cache = {},
        hue = hct.hue,
        chroma = hct.chroma,
        keyColor = hct,
  ...
  MyTonalPalette._fromHueAndChroma(this.hue, this.chroma)
      : _cache = {},
        keyColor = KeyColor(hue, chroma).create(),
        _isFromCache = false;

  MyTonalPalette._fromCache(Map<int, int> cache, this.hue, this.chroma)
      : _cache = cache,
        keyColor = KeyColor(hue, chroma).create(),
        _isFromCache = true;

  /// Create colors using [hue] and [chroma].
  static MyTonalPalette of(double hue, double chroma) {
    return MyTonalPalette._fromHueAndChroma(hue, chroma);
  }

  /// Create a Tonal Palette from hue and chroma of [hct].
  static MyTonalPalette fromHct(Hct hct) {
    return MyTonalPalette._fromHct(hct);
  }

  /// Create colors from a fixed-size list of ARGB color ints.
  ///
  /// Inverse of [TonalPalette.asList].
  static MyTonalPalette fromList(List<int> colors) {
    ...
    return MyTonalPalette._fromCache(cache, bestHue, bestChroma);
  }
